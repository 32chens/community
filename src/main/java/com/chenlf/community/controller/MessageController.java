...
public class MessageController {
    ...
    public String getNoticeList(Model model){
        ...
        //查询评论类通知
        Message message = messageService.findLastNotice(user.getId(), SystemConstants.TOPIC_COMMENT);
        if (message != null) {
            ...
            int count = messageService.findNoticeCount(user.getId(), SystemConstants.TOPIC_COMMENT);
            messageVO.put("count", count);

            int unread = messageService.findUnreadNoticeCount(user.getId(), SystemConstants.TOPIC_COMMENT);
            messageVO.put("unread", unread);
            model.addAttribute("commentNotice", messageVO);
        }
        // 查询点赞类通知
        message = messageService.findLastNotice(user.getId(), SystemConstants.TOPIC_LIKE);
        if (message != null) {
            Map<String,Object> messageVO = new HashMap<>();
            messageVO.put("message", message);
            ...
            int count = messageService.findNoticeCount(user.getId(), SystemConstants.TOPIC_LIKE);
            messageVO.put("count", count);

            int unread = messageService.findUnreadNoticeCount(user.getId(), SystemConstants.TOPIC_LIKE);
            messageVO.put("unread", unread);
            model.addAttribute("likeNotice", messageVO);
        }

        // 查询关注类通知
        message = messageService.findLastNotice(user.getId(), SystemConstants.TOPIC_FOLLOW);
        if (message != null) {
            Map<String,Object> messageVO = new HashMap<>();
            messageVO.put("message", message);
            ...
            int count = messageService.findNoticeCount(user.getId(), SystemConstants.TOPIC_FOLLOW);
            messageVO.put("count", count);

            int unread = messageService.findUnreadNoticeCount(user.getId(), SystemConstants.TOPIC_FOLLOW);
            messageVO.put("unread", unread);
